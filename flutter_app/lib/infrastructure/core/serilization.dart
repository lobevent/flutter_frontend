...
String modelToJsonString(BaseDto model) {
  final Map<String, dynamic> jsonMap = model.toJson();
  final String jsonString = _customJsonEncode(jsonMap);
  return jsonString;
}

/// Converts each model to its string representation using [modelToJsonString]
/// and than returns the model list joined with a ',' in [ ]
/// e.g. [{user:'test'},{user:'test1'}]
String modelListToJsonString(List<BaseDto> models) {
  final List<String> jsonMapList = 
    models.map((model) => modelToJsonString(model)).toList();
  return "[${models.join(',')}]";
}