...
class _CoordsPickerState extends State<CoordsPicker> {
      ...
      child: MultiBlocListener(
        listeners: [
          ...
          BlocListener<CoordsPickerCubit, CoordsPickerState>(
              listenWhen: (p, c) => p.status != c.status,
              listener: (context, state) {
                switch (state.status){case CoordsStatus.loading: this.loadingLocations = true; break;
                  case CoordsStatus.saving: this.loadingLocations = true; break;
                  case CoordsStatus.reloading:this.loadingLocations = true;break;
                  case CoordsStatus.ready:this.loadingLocations = false; locations = state.locations; break;
  ...
  Widget _buildAutocompleteAndCoords(BuildContext context) {
    return Column(
      ...
      children: [
        SizedBox(height: 80, child: _buildDropdownButton()),
        if(selectedItem != null)
          _displaySelectedLocation(),
        Visibility(child: Text("Or provide coordinates by searching for address!", style: TextStyle(color: AppColors.accentColor)), visible: selectedItem == null,),
        Visibility(
      ...
      children: [
        Expanded(
          child: DropdownButtonFormField(
              ...
              onChanged: (MyLocation? location){_data_setSelected(location!);}),
        ),
  ...
  Widget _displaySelectedLocation(){
    return ListTile(
      ...
      onTap: () {
        ...
        child: Column(children: [Text("lat: "+selectedItem!.latitude.toString()), Text("long: "+selectedItem!.longitude.toString())],),));
  }
